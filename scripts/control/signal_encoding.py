...
def train_classifier(train_data, train_labels, test_data, test_labels, mass_data = None):
    ...
    conv_layer = Conv2D(4, (2, 2), activation='relu')(traj_input)
    x = keras.layers.MaxPooling2D(pool_size=(5,2))(conv_layer)
    x = Flatten()(x)
    ...
    history = model.fit(train_data, one_hot_train_labels, epochs=90, batch_size=10, verbose=1, validation_data=(test_data,one_hot_test_labels))
...
def name_to_data():
    ...
    mass["small_"] =  0.43
    good_lookup["big_"] = [2,5,6,7,8,9,14,16,17,18,19,20,21,22,23,26,27,30]
...
def get_good_bad_traj(test = False):
    if test:
        exp_list = ["big_"]
    else:
        exp_list = ["scrape", "diverse_test_", "small_"]
...
def test_encoding():
    good_trajs, good_traj_masses, bad_trajs, bad_traj_masses = get_good_bad_traj() 
    ...
    data, labels = make_good_and_bad_dataset(num_train_good, num_train_bad, good_encoded_signals, bad_encoded_signals, good_traj_masses, bad_traj_masses, test=False)
    test_data, test_labels = make_good_and_bad_dataset(num_train_good, num_train_bad, good_encoded_signals, bad_encoded_signals, good_traj_masses, bad_traj_masses, test=True)
    model, cortical_model = train_classifier(data, labels, test_data, test_labels)
    print ("test accuracy", test_model(model, test_data, test_labels))
    def encoder(sample):
